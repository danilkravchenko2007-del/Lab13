...
void solveBaseTask() {
    ...
    int M, N;
    if (!(file >> M >> N)) {
        cout << "Ошибка: Файл пуст или содержит текст." << endl;
        return;
    }
...
int main() {
    ...
    cout << "2. Задание 3.1: Пересечение прямых" << endl;
    cin >> choice;


    cout << "-------------------------------------------------------" << endl;

    if (choice == 1) solveBaseTask();
    else if (choice == 2) solveTask3_1();
    else cout << "Ошибка: Неверный выбор!" << endl;
